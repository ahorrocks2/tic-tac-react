...
it('Should not render the Board when no players have been established', () => {
  const noPlayers = {
    X: { name: '' },
    O: { name: '' }
  };

  const wrapper = shallow(<Board board={board} players={noPlayers} />);
  expect(wrapper.find('.boardClass').length).toBe(0);
});

it('Should render Board with symbols', () => {
  const startAgain = jest.fn();
  const addSymbol = jest.fn();
  const wrapper = shallow(<Board board={board} startAgain={startAgain} addSymbol={addSymbol} draw={false} turn={X} players={players} />);
...
it('Should display a "start again" text when one symbol won', () => {
  const startAgain = jest.fn();
  const addSymbol = jest.fn();
  const wrapper = shallow(<Board board={board} won={X} startAgain={startAgain} addSymbol={addSymbol} draw={false} turn={X} players={players} />);
...
it('Should display a "start again" text when there was a draw', () => {
  const startAgain = jest.fn();
  const addSymbol = jest.fn();
  const wrapper = shallow(<Board board={board} draw={true} startAgain={startAgain} addSymbol={addSymbol} turn={X} players={players} />);
  expect(wrapper.find('p.startAgain').length).toBe(1);
});
